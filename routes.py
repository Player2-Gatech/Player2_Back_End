...
class UserMatches(Resource):
    ...
    def get(self):
        ...
        player_friends = session.query(PlayerFriend).filter(PlayerFriend.user_id_a == g.user.user_id).all()
        player_friends = list(map(lambda p: p.as_dict()['friendProfile'], player_friends))
